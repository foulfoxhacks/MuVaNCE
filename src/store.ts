...
interface State {view:'scope'|'map'|'devices'|'lab'|'settings';running:boolean;recording:boolean;liveHardware:boolean;calibrating:boolean;selectedAdapter:string|null;activeFloorId:string;floors:Floor[];samples:SignalSample[];history:SignalSample[][];calibrationFrames:SignalSample[][];calibrations:CalibrationProfile[];setView:(v:State['view'])=>void;toggleRun:()=>void;toggleRecord:()=>void;selectAdapter:(id:string|null)=>void;selectFloor:(id:string)=>void;addFloor:()=>void;renameFloor:(name:string)=>void;setFloorPlan:(data?:{name:string;dataUrl:string})=>void;replaceProject:(project:{activeFloorId:string;floors:Floor[];calibrations:CalibrationProfile[]})=>void;startCalibration:()=>void;cancelCalibration:()=>void;finishCalibration:()=>void;ingest:(frames:SignalSample[])=>void;tick:()=>void}
export const useAppStore=create<State>()(persist((set)=>({
  ...
  setFloorPlan:data=>set(s=>({floors:s.floors.map(f=>f.id===s.activeFloorId?{...f,imageData:data?.dataUrl,imageName:data?.name,imageOpacity:.42}:f)})),
  replaceProject:project=>set({activeFloorId:project.floors.some(f=>f.id===project.activeFloorId)?project.activeFloorId:project.floors[0]?.id??firstFloor.id,floors:project.floors.length?project.floors:[firstFloor],calibrations:project.calibrations,view:'map'}),
  ...
  ingest:frames=>set(s=>({samples:frames,liveHardware:true,history:[...s.history.slice(s.recording?-899:-299),frames],calibrationFrames:s.calibrating?[...s.calibrationFrames,frames]:s.calibrationFrames})),
  tick:()=>set(s=>{if(!s.running||s.liveHardware)return s;const samples=createSamples();return{samples,history:[...s.history.slice(s.recording?-899:-299),samples],calibrationFrames:s.calibrating?[...s.calibrationFrames,samples]:s.calibrationFrames}})
