...
ipcMain.handle('capture:save',async(event,frames)=>{if(!Array.isArray(frames)||frames.length>100000)throw new Error('Invalid capture');const win=BrowserWindow.fromWebContents(event.sender);const result=await dialog.showSaveDialog(win,{title:'Export signal capture',defaultPath:`muvance-capture-${new Date().toISOString().replace(/[:.]/g,'-')}.csv`,filters:[{name:'CSV signal captures',extensions:['csv']}]});if(result.canceled||!result.filePath)return false;const safe=value=>String(value??'').replace(/[\r\n,]/g,'_').replace(/^[=+\-@]/,'_$&');const rows=['timestamp,adapterId,kind,rssi,phase,amplitude,motion,channel',...frames.map(v=>[v.timestamp,safe(v.adapterId),safe(v.kind),v.rssi,v.phase,v.amplitude,v.motion,v.channel].join(','))];await writeFile(result.filePath,rows.join('\r\n'),'utf8');return true});
